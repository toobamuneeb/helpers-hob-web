  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-accent-soft via-canvas to-warm p-6 sm:p-8">
        <p className="text-sm font-medium text-ink-70">{t('home.welcomeBack')}</p>
        ...
        <p className="mt-1.5 max-w-md text-sm leading-relaxed text-ink-70">
          {t('home.findAVerifiedProfessionalNearYou')}
        </p>

        <div className="mt-5 flex flex-wrap gap-2.5">
          <Link href="/post-job"><Button size="md">{t('home.postAJob')}</Button></Link>
          <Link href="/categories"><Button size="md" variant="outline">{t('home.browseCategories')}</Button></Link>
        </div>
      </section>
      ...
      <div className="grid gap-3 xs:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        <StatTile
          label={t('home.totalSpent')} icon="wallet" tone="accent" href="/bookings"
          value={money(spend?.total_paid ?? 0, spend?.currency ?? 'EUR')}
          sub={`${spend?.total_payments ?? 0} payments`}
        />
        <StatTile
          label={t('home.inProgress')} icon="clock" tone="blue" href="/bookings"
          value={inProgress}
          sub={t('home.activeOrAwaitingConfirmation')}
        />
        <StatTile
          label={t('home.completed')} icon="check" tone="neutral" href="/bookings"
          value={completed}
          sub={t('home.jobsFinished')}
        />
        <StatTile
          label={t('home.myJobs')} icon="briefcase" tone="warm" href="/my-jobs"
          value={jobPosts}
          sub={t('home.jobPostsYouCreated')}
        />
      </div>
      ...
      >
        {shown.length === 0 ? (
          <Empty title={t('home.noCategoriesMatchThatSearch')} />
        ) : (
      ...
      >
        {providers.length === 0 ? (
          <Empty
            title={t('home.noProvidersYet')}
            sub={t('home.onceProvidersJoinInYourArea')}
          />
        ) : (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 flex items-center gap-2 text-xs text-ink-70">
                      {p.average_rating ? <span>{p.average_rating} ★</span> : <span>{t('home.new')}</span>}
