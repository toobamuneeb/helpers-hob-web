// POST /api/payments/provider-token
// Charge the provider's €5 monthly subscription token after user confirmation.
// Similar to customer payment flow - user selects card and confirms.
...
export const POST = requireRole("service_provider")(async (
  ...
  try {
    ...
    const metadata = {
      ...
      payment_kind: "provider_token",
      period,
    };

    // User selected a saved card - charge it
    if (payment_method_id) {
      logger.info("Charging provider token with selected card", {
        providerId: user.id,
        paymentMethodId: payment_method_id,
      });

      const pi = await chargePlatformOffSession({
        amountEuros: MONTHLY_TOKEN_PROVIDER,
        customerId,
        paymentMethodId: payment_method_id,
        description: `HelpersHob provider monthly token (${period})`,
        metadata,
        idempotencyKey: `ptok_${tokenPaymentId}`,
      });
    ...
    logger.info("Creating checkout for provider token (new card)", {
      providerId: user.id,
    });
    ...
    logger.error("provider-token failed", {
      userId: user.id,
      error: error.message,
    });
    return json(
      { error: "Failed to charge provider token", details: error.message },
      500
    );
  }
});
